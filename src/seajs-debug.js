...
(function(seajs, global, doc, loc) {
  ...
  function DebugPanel() {
    ...
    this.metaButtonInfo = [
      ...
      [false, "&#xe804;", "Maximize console", function() {
  ...
  DebugPanel.prototype._renderHTML = function() {
    ...
    html += '<div id="seajs-debug-map"'
      + (config.mode ? ' style="display: none;"' : '')
      + '>'
      + tmpHTML
      + '</div>'
  ...
  if (config.debug) {
    ...
    // Show console window
    config.console && new DebugPanel()

    // Add debug file mapping
    config.source && seajs.config({
      map: [
        function(uri) {
          if (/\-debug\.(js|css)+/g.test(uri)) {
            return uri
          }
          return uri.replace(/\/(.*)\.(js|css)/g, "/$1-debug.$2")
        }
      ]
    })
    ...
    // Load map
    config.mapping.length && seajs.config({
      map: config.mapping.slice(0, -1)
    })
